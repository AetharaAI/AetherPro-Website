...
const ChatInterface = ({
  ...
  const mergedResponseRef = useRef(mergedResponse);
  const setIsLoadingRef = useRef(setIsLoading);
  const setResponsesRef = useRef(setResponses);
  const setMergedResponseRef = useRef(setMergedResponse);
  ...
  useEffect(() => {
    ...
    ws.onmessage = (event) => {
      ...
      if (message.type === 'individual_response') {
        ...
        setResponsesRef.current(prev => {
          ...
          newResponses[message.agent_id] = {
            ...
            tools: message.tools || [], // Ensure tools is an array
            error: message.error,
            agent_id: message.agent_id // Ensure agent_id is part of the stored response
          };
        ...
        setMergedResponseRef.current({
          ...
          error: message.error,
          source_agents: message.source_agents || [] // Ensure source_agents is an array
        });
      ...
      } else if (message.type === 'error') { // Generic error type
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setCurrentRequestId(newRequestId); // Update state
    currentRequestIdRef.current = newRequestId; // <--- Update the ref immediately
