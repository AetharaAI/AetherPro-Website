...
const ChatHistory = ({ conversation, availableAgents, streamingResponses, isStreaming, responses }: { conversation: Conversation | null; availableAgents: Agent[]; streamingResponses: Record<string, string>; isStreaming: Record<string, boolean>; responses: Record<string, LLMResponse>; }) => {
...
const ConsolePage = () => {
  ...
  useEffect(() => {
    ...
    websocket.current = createWebSocketConnection(currentSessionId, (message) => {
      ...
      if (message.type === 'streaming_started') {
        const msg = message as StreamingStartedMessage;
        console.log('🌊 Streaming started:', msg.agent_id, 'for request:', msg.request_id);
      
      // Only handle if it's for the current request
        if (msg.request_id === currentRequestIdRef.current) {
          setIsStreaming(prev => ({ ...prev, [msg.agent_id]: true }));
          setStreamingResponses(prev => ({ ...prev, [msg.agent_id]: '' }));
          setStatusMessage(`${msg.agent_id} is streaming response...`);
        }
        return;
      }

      // === NEW: Handle response chunks ===
      if (message.type === 'response_chunk') {
        const msg = message as ResponseChunkMessage;
        ...
        if (msg.request_id === currentRequestIdRef.current) {
          ...
          setStreamingResponses(prev => ({
            ...prev,
            [msg.agent_id]: (prev[msg.agent_id] || '') + msg.chunk
          }));
        }
        return;
      }
      ...
      if (message.type === 'individual_response') {
        ...
        setIsStreaming(prev => ({ ...prev, [msg.agent_id]: false })); 
        setStatusMessage(null); 
        setResponses(prev => ({ ...prev, [msg.agent_id]: { content: msg.content, error: msg.error } }));
        setStreamingResponses(prev => {
          const updated = { ...prev };
          delete updated[msg.agent_id];
          return updated;
        }) 
      }
  ...
  const handleSubmit = async (e?: React.FormEvent) => {
    ...
    setStreamingResponses({}); setIsStreaming({}); setIsLoading(true); setResponses({}); setMergedResponse(null); setStatusMessage("Sending prompt to AetherCore...");
